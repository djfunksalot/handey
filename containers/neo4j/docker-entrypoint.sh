...
: ${NEO4J_dbms_memory_heap_initial__size:=${NEO4J_dbms_memory_heap_maxSize:-"16G"}}
: ${NEO4J_dbms_memory_heap_max__size:=${NEO4J_dbms_memory_heap_maxSize:-"16G"}}
...
#  ${exec_cmd} neo4j console
  /sbin/su-exec neo4j neo4j console
else
  /sbin/su-exec neo4j "$@"
fi
